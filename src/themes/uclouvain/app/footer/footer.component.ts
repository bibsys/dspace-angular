...
export class FooterComponent extends BaseComponent {
   ...
   * To allow remote access, `ui.ezproxy.enable` must be true AND `ui.ezproxy.proxy_url` must be configured.
   * @private
   */
  private initRemoteAccess() {
    if (this.appConfig.ui?.ezproxy.enabled && isNotEmpty(this.appConfig.ui.ezproxy?.proxy_url)) {
      this.remoteAccess.enabled = true;
      this.remoteAccess.url = this.appConfig.ui.ezproxy?.proxy_url.replace("<BASE_URL>", this.appConfig.ui.baseUrl);
    } else {
