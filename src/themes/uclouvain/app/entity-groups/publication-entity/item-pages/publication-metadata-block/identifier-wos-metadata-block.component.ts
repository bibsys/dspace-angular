...
@Component({
  template: `
      <ng-container *ngIf="item.hasMetadata('dc.identifier.isi')">
        <dt>{{ 'item.page.details.label.identifier-wos' | translate }}</dt>
        <dd><ds-generic-item-page-field [item]='item' [fields]='["dc.identifier.isi"]' [separator]="' ; '"/></dd>
      </ng-container>
  `,
  standalone: true,
  imports: [NgIf, TranslateModule, GenericItemPageFieldComponent]
})
export class IdentifierWosMetadataBlockComponent extends AbstractMetadataBlockComponent { }