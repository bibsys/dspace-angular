...
export class SubmissionUploadFilesComponent implements OnChanges, OnDestroy {
            ...
            if (sections && isNotEmpty(sections)) {
              Object.keys(sections)
                .forEach((sectionId) => {
                  ...
                  const filteredErrors = this.filterUploadErrors(sectionErrors);
                  const sectionWarning = hasValue(sectionData?.files) ? this.parseErrorsForWarning(sectionData.files, filteredErrors as any) : [];
                  ...
                  this.sectionService.isSectionType(this.submissionId, sectionId, SectionsType.Upload)
                    ...
                    .subscribe((isUpload) => {
                      if (isUpload) {
                        ...
                        if ((isEmpty(filteredErrors)) || !isEmpty(sectionWarning)) {
  ...
  /**
   * Filter upload errors to remove specific and not-relevant error messages when a file is uploaded
   * @param errors the errors return by backend
   */
  private filterUploadErrors(errors: ValidationError[]): ValidationError[] {
    return (isNotEmpty(errors))
      ? errors.filter(e => e.message !== 'error.validation.accessconditionsrequired')
      : [];
  }
  ...
  private parseErrorsForWarning(files: WorkspaceitemSectionUploadFileObject[], sectionErrors: SubmissionSectionError[]): SubmissionSectionError[] {
    return isNotEmpty(sectionErrors)
      ? sectionErrors.filter((error) => {
