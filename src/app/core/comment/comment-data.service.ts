...
export class CommentDataService extends IdentifiableDataService<Comment> implements CreateData<Comment>, SearchData<Comment>, DeleteData<Comment> {
  ...
  searchBy(searchMethod: string, options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<Comment>[]): Observable<RemoteData<PaginatedList<Comment>>> {
   ...
   * @param params List of all query argument to add into the request (the UUID of the {@link Item} parent object to which the comment will be linked)
   */
  create(comment: Comment, ...params: RequestParam[]): Observable<RemoteData<Comment>> {
    return this.createData.create(comment, ...params);
  }
