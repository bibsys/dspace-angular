    ...
    return combineLatest([this.isSubmitter$, this.isController$, this.isAdmin$]).pipe(
      ...
      map(([isSubmitter, isController, isAdmin]: [boolean, boolean, boolean]) => {
        ...
        if (isSubmitter) {
          ...
          //availableConf.push(MyDSpaceConfigurationValueType.OtherWorkspace); // NOT USED for Dial.oer
        }
        if (isController || isAdmin) {
          // availableConf.push(MyDSpaceConfigurationValueType.SupervisedItems); // NOT USED for Dial.oer
