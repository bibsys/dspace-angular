 ...
 *  2. When an event is fired on the 'affiliationFieldUpdateEvent' extract the optionList from it and load the recursive
 *     template. Each time an event is fired and handled, the select && the value behind it should be emptied based on
 *     the 'clearData' flag.
 * 
...
export class DsDynamicDepartmentAffiliationComponent extends AffiliationSelectComponent implements OnInit, OnDestroy {
  ...
  public onInput(event: any): void {
    ...
    if (isEmpty(event.target.value)) {
      this.emptySelect();  // Empty model value
